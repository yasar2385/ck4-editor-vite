			...
			return {
				...
				contents: [ {
					...
					elements: [ {
						...
						// ? LIST_MODULE_UPDATE_SIMPLE_TYPES_23-APR-22_DR_YA 
						setup: function( element ) {
							var value = element.getStyle( 'list-style-type' ) || mapListStyle[ element.getAttribute( 'type' ) ] || element.getAttribute( 'type' ) || iLIST_MODULE.setUpLabel(element) ||'';
							this.setValue( value );
						},
						commit: function( element ) {
							var value = this.getValue();
							//console.log(value);
							iLIST_MODULE.ApplyBulletStyle(element,value);
							/* if ( value ) element.setStyle( 'list-style-type', value );
							else element.setStyle( 'list-style-type', 'none' ); */
						}
					} ]
				} ],
				onShow: function() {
				...
				contents: [ {
					...
					elements: [ {
						...
						},
						{
							...
							setup: function( element ) {
								var value = element.getStyle( 'list-style-type' ) || mapListStyle[ element.getAttribute( 'type' ) ] || element.getAttribute( 'type' ) || iLIST_MODULE.NumberStyle(element,null,{show:!0}) ||'';
								console.log(value);
								this.setValue( value );
							},
							commit: function( element ) {
								var value = this.getValue();
								//console.log(getListType(element.getAttribute( 'list-content' ),COMMIT,value));
								iLIST_MODULE.NumberStyle(element,value,{change:!0});
								/* var eType=element.getAttribute( 'list-content' );
								if(eType!=null){
									value = getListType(eType,COMMIT,value);
