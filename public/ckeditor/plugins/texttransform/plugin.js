        ...
        init: function(editor)
        {
            ...
            var transformSelectedText = function (editor, tag, transformFunc) {
                ...
                var startElement = editor.getSelection().getStartElement();
					if (startElement.getName() == 'sc' && startElement.hasAttribute('data-name') ){
						if (startElement.getAttribute('data-name') == tag){//same apply case
							...
							if (startElement.$.innerText.length == selected_text.length){
								...
								var bookmarks = curselection.createBookmarks(true);
								startElement.remove(true);
								var range = selection.getRanges()[0];
								range.moveToBookmark(bookmarks[0]);
								range.select();
								var style = new CKEDITOR.style(FormattingHandler.rule.sc);
            ...
            editor.addCommand('smallcaps',
                {
                    exec: function () {
						// alert('ok');
                        // var selection = editor.getSelection();
						transformSelectedText(editor, 'sc', function (text) {
                            if (editor.langCode == "tr") {//Added By Durai
            ...
            editor.ui.addButton('SmallCaps',
                {
                    /* 06_JAN_2023_YA/RR */
                    label:"Small Caps", /* editor.lang.texttransform.transformTextSwitchLabel */
                    command: 'smallcaps',
                    icon: this.path + 'images/transformCapitalize.png',/* images/transformCapitalize.png */
