 ...
 * siva added try catch for insering latex equation temporary for 17th demo
 * added method _IgnoreAttribute to ignore track changes for "data-track-changes-ignore" attribure
 */
(function (a) {
    a.ice = {}
}(this || window));
...
(function (q, e) {
    ...
    h.prototype = {
        ...
        insert: function (I) {
            ...
            I = I || {};
            try {
                // Safari Add new keyword and click outside keyword and click & entering text inserted into TC issue - 12_DEC_2022_AN
                if (D && D.startContainer) {
                    const {
                        startContainer
                    } = D;
                    // Check if the startContainer is part of a keyword group 
                    if (startContainer.classList && startContainer.classList.contains('kwd-group')) {
                        const activeKeyword = startContainer.querySelector('.kwd.active');

                        if (activeKeyword.textContent === 'keyword' && CKEDITOR.env.safari) {
                            const keywordElement = GlobalEditor.document.getById(activeKeyword.getAttribute('id'));
                            GlobalEditor.getSelection().selectElement(keywordElement);
                            D = this.getCurrentRange();
                        }
                    } else if (
                        startContainer.textContent === 'keyword' &&
                        startContainer.parentElement && startContainer.parentElement.classList.contains('kwd', 'impactdeltag', 'active') &&
                        CKEDITOR.env.safari
                    ) {
                        // Handle Safari-specific issue with keyword text insertion 29_DEC_2024 - refactor
                        // Safari Add new keyword and entering text inserted into TC issue - 06_DEC_2022_AN 
                        const range = GlobalEditor.getSelection().getRanges()[0];
                        range.deleteContents();
                        range.select();
                        // G = false; // Uncomment if needed
                    }
                }
                // ? 07_MAY_2024_YA_PASTE_ON_TEMPLATE_AREA - FOR NEWLY INSERTED TEXT
                let IsInserted = (typeof PlaceHolder_Module != "undefined" ? PlaceHolder_Module.CHECK_PASTE_SELECTION(D) : false);
                if (G) {
                    if (!IsInserted) this._deleteContents(false, D);
                    D = this.getCurrentRange();
                }
                if (D || B) {
                    var A = I.nodes;
                    ...
                    // Impact Cursor movement in Insert/del tag - Anand
                    if (!!D.startContainer && (D.startContainer.nodeName.toLowerCase() == 'insert' || D.startContainer.nodeName.toLowerCase() == 'del' || !!D.startContainer.parentNode && D.startContainer.parentNode.nodeName.toLowerCase() == 'del')) {
        ...
        _deleteContents: function (D, B) {
            ...
            /*Ignore XREF element inside insert tag 22-nov-22 IMPACT_AN*/
            if (B && (B.startContainer.firstChild != null || B.startContainer.lastChild != null)) {
                let first = B.startContainer.firstChild,
                    last = B.startContainer.lastChild;
                if (last.nodeName == 'A' && last.classList.contains('xref') || first.nodeName == 'A' && first.classList.contains('xref')) {
            ...
            /*else if(B.startContainer.nextSibling!=null&&B.startContainer.nextSibling.getAttribute('data-class')=='ckcommentsfull'||B.startContainer.previousSibling!=null&&B.startContainer.previousSibling.getAttribute('data-class')=='ckcommentsfull'){
            	TOASTER_ALERT('Ignore_KeyEvent_NoteQry',{type:'warning'});
            	return true;
            }*/
            try {
                if (B && B.collapsed === false) {
                    ...
                    //?3349041 OUP_J_INDE_169 if the Text selection present we will track the issue by DR 09-03-2025
                    var checkOne = this._isCurrentUserIceNode(this._getIceNode(B.startContainer, s));
                    var checkTwo = B.startOffset == B.endOffset && B.startOffset <= B.startContainer.textContent.length;
                    if (checkOne && !(D && checkTwo)) {
                        return false;
                    }
                    if (D) {
                        if (C.type === "mozilla") {
        ...
        _isCurrentUserIceNode: function (B) {
            // ? IMPACT on insert text in newly inserted table caption break issue.
            var A = Boolean(B && e(B).attr(this.attributes.userId) === String(this.currentUser.id) || !!$(B).find('.caption').length);
            if (A && this._sessionId) {
                A = B.getAttribute(this.attributes.sessionId) === String(this._sessionId) || !!$(B).find('.caption').length;
            }
            return A
        },
        ...
        _addChange: function (D, F, C) {
            ...
            if (!this._changes[E]) {
                ...
                this._changes[E] = {
                    ...
                    datarole: (USER_INFO.TRACK_ROLE_NAME),
        ...
        _addNodeToChange: function (E, D) {
            ...
            // IMPACT RECORD THE ROLE DETAILS INSERT/DELETE TAG BY DR 12-06-2024
            if (!D.getAttribute(this.attributes.datarole)) B[this.attributes.datarole] = F.datarole
            // IMPACT RECORD THE ROLE DETAILS INSERT/DELETE TAG BY DR 12-06-2024
            if (C == F.userid) {
            ...
            if (D) {
                ...
                if (T) {
                    ...
                    if (L) {
                        ...
                        M.setEnd(C, 1);
                        M.collapse()*/
                        // Space is inserted blue line is not occuered - 12-DEC-22_AN
                        E = true;
                        if (L == ' ' && R.textContent.length == 0) {
                            L = "&nbsp;"
                        }
                        var C = Z.createTextNode(L);
                        L == '&nbsp;' && R.textContent.length == 0 ? $(R).html(C.textContent) : R.appendChild(C);
                        $(R).html() == '&nbsp;' && R.textContent.length == 1 ? M.setEnd(R, 1) : M.setEnd(C, 1);
                        M.collapse();
                    } else {
                        x(R, M, Z, X)
                    }
        ...
        _deleteSelection: function (H) {
            ...
            for (var G = 0; G < A.length; G++) {
                var D = A[G];
                /*IMPACT SELECT MATH WITH TEXT AND PRESS DELETE KEY START  - YA_AN*/
                /* YA_TH  07-Apr-2022 */
                if (iKEY_EVENT_HANDLING.CHECK_IGNORE_LITE_TRACK(D)) {
                    continue;
                }
                /*IMPACT SELECT MATH WITH TEXT AND PRESS DELETE KEY END - YA_AN*/
                if (!D || !D.parentNode) {
                    continue
                }
                ...
                if (!this._getVoidElement({
                    ...
                    if (D.nodeType !== q.dom.TEXT_NODE) {
                        ...
                        if (q.dom.hasNoTextOrStubContent(D)) { // ? impact ignore-ness
                            if ((D.localName == 'span') && (D.getAttribute('data-class') == 'ckcommentsfull')) {
                                AlertNewDialog.fire('warning', 'Note', 'Ignore_KeyEvent_NoteQry', 'OK', '');
                                continue
                            }; //? impact ignoreness end */
                            this._removeNode(D);
                            continue
                        }
            ...
            if (J.length) {
                ...
                H.collapse(false); // True to False for Impact Cursot Movement Issue
        ...
        _deleteLeft: function (I) {
            var E = q.dom.isBlockElement(I.startContainer) && I.startContainer || q.dom.getBlockParent(I.startContainer, this.element) || null,
                ...
                P, M;
            // if((I.commonAncestorContainer.parentNode.tagName == 'A') || ((M != undefined) && (M.tagName == 'A'))){ // Delete content 'A'
            //     return null;
            // }
            if (H) {
                return false
            ...
            if (C === 0 || Q.nodeType !== q.dom.TEXT_NODE) {
                ...
                /*IMAPCT PLACE CURSOR AFTER MATH AND PRESS BACKSPACE KEY START - ANAND*/
                if ($(M).hasClass('inline-formula')) {
                    console.log('from lite plugins');
                    TOASTER_ALERT('Ignore_KeyEvent_Math', {
                        type: 'warning'
                    });
                }
                /*IMAPCT PLACE CURSOR AFTER MATH AND PRESS BACKSPACE KEY END- ANAND*/
        ...
        _addDeleteTracking: function (D, J) {
            let D_Par = D.parentNode,
                D_GPar = D.parentNode.parentNode;
            let IsMath = $(D_GPar).hasClass('cke_widget_mathjax') ? !0 : !1;
            let D_TagName = D_Par.localName;
            var link_Class_Arr = ['uri', 'email', 'ext-link'];
            let IsLink = (D_TagName == 'span' && link_Class_Arr.some(cls => $(D_Par).hasClass(cls))) ? true : false;
            let Xref = (D_TagName == 'sup' && $(D_Par).find(".xref").length > 0 || $(D).parents('a.xref').length > 0) ? (true) : (false);
            //if ((D_Par.localName == 'a') || (D_GPar.localName == 'a') || ((D_Par.localName=='sup') && (D_GPar.getAttribute('class') == 'aff')) || $(D_GPar).hasClass('cke_widget_mathjax')|| (D_Par.localName == 'span') && (D_Par.getAttribute('class') == 'label')){
            if (((['a'].includes(D_TagName) || Xref) || (D_Par.getAttribute('class') == 'label')) || IsMath || IsLink) {
                /* // ! [2398448]: Replace the timer in dialog box with an OK button */
                if (IsMath) {
                    TOASTER_ALERT('Ignore_KeyEvent_Math', {
                        type: 'warning'
                    ...
                    TOASTER_ALERT('Ignore_hyperlink_text', {
                        type: 'warning'
                    });
                } else {
                    // TOASTER_ALERT('Ignore_KeyEvent_XREFS', {type: 'warning'});
                    AlertNewDialog.fire('warning', "Warning", 'Ignore_KeyEvent_XREFS', 'OK', '', true, {
                        override: false
                    });
                }
        ...
        handleKeyDown: function (A) {
            ...
            /*Start: Affiliation -- Added by ak on 08-07-2020*/
            if (this._IgnoreAttribute()) {
                return true
            }
            /*End: Affiliation -- Added by ak on 08-07-2020*/
        ...
        /*Start: Affiliation -- Added by ak on 08-07-2020*/
        _IgnoreAttribute: function () {
            var sel = GlobalEditor.getSelection();
            sel = sel.getStartElement();
            if (sel == null) {
                return;
            }
            if (sel.hasAttribute('data-track-changes-ignore') == true) {
                return true;
            }
        },
        /*End: Affiliation -- Added by ak on 08-07-2020*/
        ...
        prepareToCut: function () {
            ...
            try {
                ...
                //Safari Cut Text with comments/query - 03_DEC_22_AN
                var range = GlobalEditor.getSelection().getRanges()[0];
                var CloneCont = range.cloneContents().$;
                if (CKEDITOR.env.safari && $(CloneCont).find('[data-class=ckcommentsfull]').length > 0) {
                    range.deleteContents();
                    range.select();
                }
            }
    ...
    b.prototype = {
        selectStartAndCollapse: function () {
            if (this.start) {
                ...
                /*Impact Cursor movement in Insert/del tag - start Anand*/
                if (!!f.startContainer.parentElement && f.startContainer.parentElement.nodeName.toLowerCase() != 'insert') {
                    f.setStartBefore(this.start);
                    f.collapse(true);
                }
                /*Impact Cursor movement in Insert/del tag - End Anand*/
        ...
        i.prototype._ensureViewportContainment = function (z) {
            ...
            if (B.width >= this.dimensions.width) {
                if (x[0]) {
                    E = true;
                    switch (x[0]) {
                        case this.STICKS_OUT_RIGHT: //Modified by ak on 14-08-2020 for tooltip Issue Fixes
                            C.setHorizontal("right");
                            if (this.options.targetJoint) {
                                F.setHorizontal("right")
                            }
                            break;
                        case this.STICKS_OUT_LEFT: //Modified by ak on 14-08-2020 for tooltip Issue Fixes
...
(function (j, v) {
    ...
    function f(G) {
        try {
            return G && G.className && c.test(G.className)
        } catch (e) {
            console.log(e);
        }
    }
    ...
    h.prototype = {
        init: function (O, K) {
            ...
            if (K.contextMenu !== false) {
                ...
                if (O.contextMenu) {
                    O.contextMenu.addListener((function (T) { // Yasar added rule
                        if (T && this._tracker && this._tracker.currentChangeNode(T) && (!USER_INFO.IS_AUTHOR)) {
                            var S = {};
        ...
        _onBeforeCommand: function (H) {
            ...
            if ("cut" === G) {
                // ! 08_MAR_2024_YA
                if (typeof iKEY_EVENT_HANDLING != "undefined" && !iKEY_EVENT_HANDLING.RESTRICT_CUT(H.data)) return false;
