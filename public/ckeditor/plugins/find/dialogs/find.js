...
(function() {
    ...
    var pages = ['find', 'replace'],
        ...
        buttonsMapping = {
            "find": ["btnFind"],
            "replace": ["btnFindReplace", "btnReplaceAll", "btnFindNext"]
        };
    ...
    function findDialog(editor) {
        ...
        characterRange.prototype = {
            ...
            removeHighlight: function() {
                if (!this._.highlightRange)
                    return;
                // REMOVE BG-COLOR SK
                // ? MODIFY BY 24_JUNE_2024_YA                
                editor.fire('lockSnapshot');
                let selector = 'span[data-cke-highlight]';
                editor.document.find(selector).toArray().forEach((item) => {
        ...
        var finder = {
            ...
            find: function(pattern, matchCase, matchWord, matchCyclic, highlightMatched, cyclicRerun) {
                ...
                while (character !== null) {
                    ...
                    this.matchResult = matchState == 2 ? !0 : !1;
                    if (matchState == KMP_MATCHED) {
                        this.matchCount++;
                        if (matchWord) {
                        ...
                        let case_i = this.matchRange._.highlightRange != null && this.matchRange._.highlightRange.startContainer.getName() == 'del',
                            arugs = Array.prototype.slice.call(arguments),
                            IsVisible = false;
                        if (highlightMatched !== false) {
                            this.matchRange.highlight();
                        }
                        if (case_i) {
                            // ? Ignore finding del tag - Impact
                            this.matchRange.removeHighlight();
                            //this.matchRange = null;
                            return finder.find.apply(this, arugs.concat([true]));
                        }
                        IsVisible = !!this.matchRange._.highlightRange ? this.matchRange._.highlightRange.startContainer.isVisible() : false;
                        // ?  impact - ANAND - check weather element is visible or hidden
                        return IsVisible ? true : (arugs.length < 5 ? finder.find.apply(this, arugs.concat([true])) : console.log(arugs));
                        // ? impact loop running on replace all - Anand
                    }
                }
            ...
            replace: function(dialog, pattern, newString, matchCase, matchWord, matchCyclic, isReplaceAll) {
                ...
                if (this.matchRange && this.matchRange.isMatched() && !this.matchRange._.isReplaced && !this.matchRange.isReadOnly() && !matchOptionsChanged) {
                    ...
                    //? IMPACT SIVA MODIFYED - YA_30_APR_2024
                    if (iFIND_REPLACE.IsTrackRange(domRange, this) || (!isReplaceAll && (iFIND_REPLACE.IsIgnoreArea(domRange, this) || iFIND_REPLACE.IsIgnoreArea(editor, this))) || (!!isReplaceAll && iFIND_REPLACE.IsIgnoreArea(domRange, this))) {
                        if (!iFIND_REPLACE.IsTrackRangeBool && !isReplaceAll) TOASTER_ALERT('FIND_REPLACE_IGNORE', {
                            type: 'warning'
                        });
                        if (!!isReplaceAll) iFIND_REPLACE.replaceRestrict = true;
                    ...
                    // ? IMPACT modified
                    if (!isReplaceAll) {
                        window.parent.IMPACT_SELECTION._SNAPSHOT({
                            lock: !0
                        });
                        var selection1 = editor.getSelection();
                        var bookmarks = selection1.createBookmarks(true);
                        // ? IMPACT ? 09-Dec-202 YA EMPTY_REPLACE
                        if (newString == "") editor.insertElement(editor.document.createElement('span'));
                        else editor.insertText(newString);
                        var range = selection1.getRanges()[0];
                        iFIND_REPLACE.AddToTrack(bookmarks, range)
                        ...
                        domRange.select();
                        var book_marks = editor.getSelection().createBookmarks();
                        // ? EMPTY REPLACE ALL
                        window.parent.IMPACT_SELECTION._SNAPSHOT({
                            lock: !0
                        });
                        ...
                        iFIND_REPLACE.AddToTrack(book_marks, range)
                        editor.getSelection().selectBookmarks(book_marks);
                        window.parent.IMPACT_SELECTION._SNAPSHOT({
                            unlock: !0
                        });
                    }
        ...
        return {
                            ...
                            children: [{
                                    ...
                                    onKeyDown: function(evt) {
                                        ...
                                        if (keystroke !== ENTER_KEY) {
                                            EnableDisableBtns(this.getDialog());
                                            return;
                                        }
                                        Find_Loop_fn(this.getDialog());
                                    },
                    ...
                    elements: [{
                            ...
                            children: [{
                                    ...
                                    onKeyDown: function(evt) {
                                        ...
                                        // Find_Loop_fn(this.getDialog(), evt);
                                    },
                                ...
                                },
                                {
                                    ...
                                    onClick: function() {
                                        ...
                                        iFIND_REPLACE.replaceRestrict = !1;
                                        if (!!finder.replaceAllChkBox) {
                                            return;
                                        }
                                        var dialog = this.getDialog(),
                                            replaceResult = execReplace(dialog);
                                        iFIND_REPLACE.RECORD_HANDLE('replace', dialog.getValueOf('replace', 'txtFindReplace'), dialog.getValueOf('replace', 'txtReplace'));
                                        if (replaceResult == false) {
                                            TOASTER_ALERT(iFIND_REPLACE.replaceRestrict ? 'FIND_REPLACE_IGNORE' : 'Specified_text', {
                                                type: 'warning'
                                            });
                                        }
                                    }
                        ...
                        },
                        {
                            ...
                            children: [{
                                    ...
                                    onKeyDown: function(evt) {
                                        ...
                                        if (keystroke !== ENTER_KEY) {
                                            EnableDisableBtns(this.getDialog(), null, {
                                                id: 'txtReplace',
                                    ...
                                    onChange: function(evt) {
                                        setTimeout((dialog, _Id) => {
                                            EnableDisableBtns(dialog, null, {
                                                id: 'txtReplace',
                                                target: document.getElementById(_Id) || null
                                            });
                                        }, 100, this.getDialog(), evt.sender._.inputId);
                                    }
                                },
                                {
                                    type: 'button',
                                    ...
                                    onClick: function() {
                                        ...
                                        if (finder.replaceCounter) {
                                            ...
                                            TOASTER_ALERT('FIND_REPLACE_PASS', {
                                                addText: lang.replaceSuccessMsg.replace(/%1/, finder.replaceCounter)
                                            });
                                            editor.fire('saveSnapshot');
                                        } else {
            ...
            onLoad: function() {
                ...
                this.selectPage = CKEDITOR.tools.override(this.selectPage, function(originalFunc) {
                    return function(pageId) {
                        ...
                        // ? IMPACT_YA_30_APR_2024
                        EnableDisableBtns(dialog, pageId)
                    };
                });
            ...
            onShow: function() {
                ...
                field.select();
                EnableDisableBtns(this, startupPage); // ? IMPACT_YA_30_APR_2024
                this[(startupPage == 'find' && this._.editor.readOnly ? 'hide' : 'show') + 'Page']('replace');
                if (typeof iFIND_REPLACE != "undefined") {
                    iFIND_REPLACE.showLoop(this, startupPage, finder);
                    // setTimeout((dialog, tab) => {
                    //     EVENT_INVOKE(showItems, dialog, tab);
                    // }, 1000, this.getDialog(), startupPage)
                }
            },
            onHide: function() {
                var range;
                ...
                iFIND_REPLACE.ShowLoopBool = false;
            },
            onFocus: function() {
        ...
        function get_values_alert(dialog, tab, Options = {}, finderObj) {
            if (!finderObj) finderObj = finder
            let index = pages.indexOf(tab),
                returnValues = {};
            fieldsMapping.forEach(function(arr, idx, arr) {
                let val = arr[idx][index],
                    key = val.replace("txtFind", "").replace("txtReplace", "");
                returnValues[key] = dialog.getValueOf(tab, val);
            });
            if (Options.alert && finderObj) {
                if (!finderObj.find(returnValues[tab == "find" ? 'Find' : 'Replace'], returnValues['CaseChk'], returnValues['WordChk'], true, true)) {
                    TOASTER_ALERT('Specified_text', {
                        type: 'warning'
                    });
                }
            ...
            /* 
            {
            	'Find': dialog.getValueOf(tab, 'txtFindFind'),
            	"CaseChk": dialog.getValueOf(tab, 'txtFindCaseChk'),
            	"WordChk": dialog.getValueOf(tab, 'txtFindWordChk'),
            	"Cyclic": dialog.getValueOf(tab, 'txtFindCyclic'),
            }
             */
        }

        function EnableDisableBtns(dialog, tab, Options = {}) {
            // ? IMPACT_YA_30_APR_2024
            if (!tab) tab = dialog._.currentTabId;
            setTimeout((dialog, tab) => {
                let IsEmpty = (dialog.getValueOf(tab, (tab == "find" ? 'txtFindFind' : 'txtFindReplace')) == "");
                if (Options.target) IsEmpty = Options.target.value == "";
                // if (tab == "find") IsEmpty = (dialog.getValueOf(tab, 'txtFindFind') == "");
                // else IsEmpty = (dialog.getValueOf(tab, 'txtFindReplace') == "");
                if (buttonsMapping[tab]) {
                    buttonsMapping[tab].forEach(btn => {
                        if (dialog.getContentElement(tab, btn))
                            dialog.getContentElement(tab, btn)[IsEmpty ? 'disable' : 'enable']();
                    });
                }
                iFIND_REPLACE.SHOW_HISTORY(dialog, tab, IsEmpty, Options);
            }, 100, dialog, tab);
            if (typeof iFIND_REPLACE['EnableDisableBtns'] == "undefined") {
                iFIND_REPLACE['EnableDisableBtns'] = EnableDisableBtns;
            }
        }

        function EVENT_INVOKE(divGroup, dialog, tab, self) {
            self = iFIND_REPLACE;
            try {
                let {
                    find_input_filed,
        ...
        function Find_Loop_fn(dialog, tab, evt) {
            if (!tab) tab = dialog._.currentTabId;
            var {
                Find,
                Replace,
                ...
                WordChk,
                Cyclic
            } = get_values_alert(dialog, tab),
                findresult = null, FIND_VAL = Find ? Find : Replace;
            if (typeof iFIND_REPLACE != "undefined") {
                // ? 20_APR_2024_YA - SEARCH HISTORY
                iFIND_REPLACE.RECORD_HANDLE(tab, FIND_VAL);
            }
            findresult = finder.find(FIND_VAL, CaseChk, WordChk, Cyclic);
            if (!!finder.findEnd) {
                finder.findEnd = false;
                findresult = finder.find(FIND_VAL, CaseChk, WordChk, Cyclic);
            }
            if (!finder.matchResult && !Cyclic) {
                if (!!finder.matchCount) {
                    finder.matchCount = 0;
                    AlertNewDialog.fire('warning', '', 'find_cyclic', 'Yes', 'No').then((result) => {
            ...
            } else if (!findresult && !!Cyclic) {
                AlertNewDialog.fire('warning', '', 'find_ended', 'OK', '');
                finder.findEnd = true;
            }
        }
